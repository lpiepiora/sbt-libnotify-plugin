...
javahClasses := Seq("it.paperdragon.libnotify.LibNotify$")
...
def standardJavaIncludes(javaHome: File): String =
...
lazy val compileNativeTask = Def.task {
  ...
  if (exitCode != 0) error(s"Compilation failed. Exit code: $exitCode") else outputFile
}
